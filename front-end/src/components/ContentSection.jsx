...
const ContentSection = () => {
  ...
  const handleEdit = () => {
    fetch(`http://localhost:3000/api/class/${selectedClass.id}`, {
      method: "PUT",
      headers: {
      ...
      .then((data) => {
        if (data.isSuccessful) {
          ...
          console.error("Error editing class:", data);
        }
      })
    ...
    <div>
      <div className="StudentContent w-full h-auto bg-white grid grid-cols-1 sm:grid-cols-2 min-[760px]:grid-cols-2 min-[1000px]:grid-cols-3 min-[1920px]:grid-cols-5 mx-0 px-0 py-5 gap-4">
        {classes.map((classItem, index) => (
          <div
            key={index}
            className="bg-[#dadde2] p-4 rounded-lg shadow-md flex flex-row items-center h-[150px] mx-4 justify-between border border-white"
          >
            <div className="flex flex-col flex-grow">
              <Link to={`/class/${classItem.id}/details`}>
                <h3 className="text-xl font-bold mb-2 text-slate-500 hover:scale-105 duration-300">
                  Class Name: {classItem.name}
                </h3>
              </Link>
              <p className="text-slate-500">ID: {classItem.id}</p>
            </div>
